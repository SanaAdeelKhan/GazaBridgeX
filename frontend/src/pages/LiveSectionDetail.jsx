          ...
          <div className="flex justify-center mb-4">
            <motion.div
              ...
              className={`w-16 h-16 rounded-full flex items-center justify-center ${
                type === 'delete' ? 'bg-red-100' : 'bg-orange-500'
              }`}
            >
              {type === 'delete' ? (
...
export default function LiveSectionDetail() {
  ...
  if (loading) return <div className="pt-24 min-h-screen flex items-center justify-center"><div className="animate-spin w-12 h-12 border-4 border-orange-400 border-t-transparent rounded-full" /></div>;
  if (error || !liveSection) return <div className="pt-24 min-h-screen flex items-center justify-center"><div className="text-center"><h2 className="text-2xl font-bold text-gray-900 mb-4">Not found</h2><Link to="/live-sections" className="text-orange-600 font-semibold">← Back</Link></div></div>;

  return (
    <div className="pt-24 min-h-screen bg-gradient-to-br from-orange-600 via-white to-orange-500">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <Link to="/live-sections" className="inline-flex items-center gap-2 text-orange-600 font-semibold mb-6">← Back</Link>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="bg-white rounded-3xl shadow-xl p-8 mb-8">
            <div className="flex justify-between items-start mb-6">
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${effectiveStatus === 'active' ? 'bg-orange-500 text-orange-600 border-orange-400' : 'bg-red-100 text-red-700 border-red-200'}`}>{isEnded ? 'Ended' : effectiveStatus}</span>
            ...
            <div className="flex items-center gap-3 pt-4 border-t">
              <div className="w-10 h-10 bg-gradient-to-br from-orange-600 to-orange-500 rounded-full flex items-center justify-center text-white font-bold">{liveSection.user_full_name?.split(' ').map(n => n[0]).join('')}</div>
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8">
            <div className="flex justify-between items-center mb-6">
              ...
              {canAddContent && <button onClick={() => setShowAddContentModal(true)} className="px-4 py-2 bg-gradient-to-r from-orange-600 to-orange-500 text-white rounded-xl font-semibold text-sm">Add Content</button>}
            </div>
              ...
              <div className="space-y-4">
                {liveSection.contents?.map((c, i) => (
                  <div key={c.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-xl">
                    <div className="w-8 h-8 bg-orange-500 text-orange-600 rounded-full flex items-center justify-center font-bold text-sm">{i + 1}</div>
                    <div className="flex-1"><h3 className="font-semibold">{c.content_title}</h3>{c.description && <p className="text-sm text-gray-600 mt-1">{c.description}</p>}<a href={c.link} target="_blank" rel="noopener noreferrer" className="text-orange-600 text-sm mt-2 inline-block">Open →</a></div>
