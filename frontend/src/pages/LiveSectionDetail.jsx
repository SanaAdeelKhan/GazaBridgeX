...
export default function LiveSectionDetail() {
  ...
  if (loading) return <div className="pt-24 min-h-screen flex items-center justify-center"><div className="animate-spin w-12 h-12 border-4 border-[#C97B1A] border-t-transparent rounded-full" /></div>;
  if (error || !liveSection) return <div className="pt-24 min-h-screen flex items-center justify-center"><div className="text-center"><h2 className="text-2xl font-bold text-gray-900 mb-4">Not found</h2><Link to="/live-sections" className="text-[#C97B1A] font-semibold">← Back</Link></div></div>;

  return (
    <div className="pt-24 min-h-screen bg-gradient-to-br from-[#fdf3e3] via-white to-[#fdf3e3]">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <Link to="/live-sections" className="inline-flex items-center gap-2 text-[#C97B1A] font-semibold mb-6">← Back</Link>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="bg-white rounded-3xl shadow-xl p-8 mb-8">
            <div className="flex justify-between items-start mb-6">
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${effectiveStatus === 'active' ? 'bg-[#fdf3e3] text-[#1a2e1a] border-emerald-200' : 'bg-red-100 text-red-700 border-red-200'}`}>{isEnded ? 'Ended' : effectiveStatus}</span>
            ...
            <div className="flex items-center gap-3 pt-4 border-t">
              <div className="w-10 h-10 bg-gradient-to-br from-[#e18f23] to-[#E8920F] rounded-full flex items-center justify-center text-white font-bold">{liveSection.user_full_name?.split(' ').map(n => n[0]).join('')}</div>
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8">
            <div className="flex justify-between items-center mb-6">
              ...
              {canAddContent && <button onClick={() => setShowAddContentModal(true)} className="px-4 py-2 bg-[#e18f23] hover:bg-[#c97a18] text-white rounded-xl font-semibold text-sm">Add Content</button>}
            </div>
              ...
              <div className="space-y-4">
                {liveSection.contents?.map((c, i) => (
                  <div key={c.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-xl">
                    <div className="w-8 h-8 bg-[#fdf3e3] text-[#1a2e1a] rounded-full flex items-center justify-center font-bold text-sm">{i + 1}</div>
                    <div className="flex-1"><h3 className="font-semibold">{c.content_title}</h3>{c.description && <p className="text-sm text-gray-600 mt-1">{c.description}</p>}<a href={c.link} target="_blank" rel="noopener noreferrer" className="text-[#C97B1A] text-sm mt-2 inline-block">Open →</a></div>
