    ...
    return (
      <div className="pt-24 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/courses" className="text-orange-600 font-semibold">← Back to Courses</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-24 min-h-screen bg-gradient-to-br from-orange-600 via-white to-orange-500">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <Link to="/courses" className="inline-flex items-center gap-2 text-orange-600 font-semibold mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="bg-white rounded-3xl shadow-xl p-8 mb-8">
            <div className="flex justify-between items-start mb-6">
              <div>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${course.status === 'active' ? 'bg-orange-500 text-orange-600' : 'bg-gray-100 text-gray-700'}`}>
                  {course.status}
                </span>
            ...
            <div className="flex items-center gap-3 pt-4 border-t">
              <div className="w-10 h-10 bg-gradient-to-br from-orange-600 to-orange-500 rounded-full flex items-center justify-center text-white font-bold">
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8">
            <div className="flex justify-between items-center mb-6">
              ...
              {canAddContent && (
                <button onClick={() => setShowAddContentModal(true)} className="px-4 py-2 bg-gradient-to-r from-orange-600 to-orange-500 text-white rounded-xl font-semibold text-sm">
                  Add Content
                </button>
              )}
            </div>
              ...
              <div className="space-y-4">
                {course.contents?.map((content, index) => (
                  <div key={content.id} className="flex items-start gap-4 p-4 bg-gray-50 rounded-xl">
                    <div className="w-8 h-8 bg-orange-500 text-orange-600 rounded-full flex items-center justify-center font-bold text-sm">
                      {index + 1}
                    </div>
                    <div className="flex-1">
                      ...
                      <a href={content.link} target="_blank" rel="noopener noreferrer" className="text-orange-600 text-sm mt-2 inline-block">
                        Open Resource →
                      </a>
                    </div>
