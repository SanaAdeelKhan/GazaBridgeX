...
const CATEGORY_ICONS = { learn_language: '🗣️', learn_tech_ai: '🤖', career_cv_help: '💼', mental_health_support: '🧠', academic_tuition: '📖', creative_skill: '🎨', others: '📌' };
const CATEGORY_LABELS = { learn_language: 'Learn a Language', learn_tech_ai: 'Learn Tech / AI', career_cv_help: 'Career / CV Help', mental_health_support: 'Mental Health Support', academic_tuition: 'Academic Tuition', creative_skill: 'Creative Skill', others: 'Others' };
const AVAILABILITY_LABELS = { '1_2_hours': '1-2 hours/week', '3_5_hours': '3-5 hours/week', '6_8_hours': '6-8 hours/week', '8_10_hours': '8-10 hours/week', '10_plus_hours': '10+ hours/week' };
...
export default function OfferDetail() {
  ...
  const fetchOffer = async () => {
    ...
    } catch (err) { setError('Failed to load offer'); }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate('/posts');
    } catch (err) { alert(err.response?.data?.detail || 'Failed to delete offer'); }
  };
  ...
  return (
    <div className={`pt-24 min-h-screen ${tw.pageBg}`}>
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className={`${tw.card} rounded-3xl shadow-xl p-8 mb-8`}>
            ...
            <p className={`mb-6 leading-relaxed ${tw.bodyText}`}>{offer.description}</p>
          ...
          <div className={`${tw.card} rounded-3xl shadow-xl p-8 mb-8`}>
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-2xl font-bold ${tw.titleText}`}>Linked Courses ({linkedCourses.length})</h2>
              {canLink && (
          ...
          {/* Linked Live Sections */}
          <div className={`${tw.card} rounded-3xl shadow-xl p-8`}>
            <div className="flex justify-between items-center mb-6">
              <h2 className={`text-2xl font-bold ${tw.titleText}`}>Linked Live Sections ({linkedLiveSections.length})</h2>
              {canLink && (
            ...
            {linkedLiveSections.length === 0 ? (
              <div className="text-center py-8">
                <div className="text-4xl mb-4">📡</div>
                <p className={tw.bodyText}>No live sections linked to this offer yet.</p>
                {canLink && (
                  <button onClick={() => setShowLinkLiveSectionModal(true)} className="mt-4 text-purple-600 font-semibold text-sm hover:text-purple-700">
                    Link a live section →
                  </button>
                )}
              </div>
              ...
              <div className="space-y-3">
                {linkedLiveSections.map(ls => (
                  <Link key={ls.id} to={`/live-sections/${ls.id}`}
                    className="block p-4 bg-white/60 rounded-xl hover:bg-white/90 transition-colors group">
                    <div className="flex items-start justify-between">
                      <div>
      ...
      <AnimatePresence>
        {showLinkModal && <LinkCourseModal offerId={id} onClose={() => setShowLinkModal(false)} onLinked={fetchOffer} />}
        {showLinkLiveSectionModal && <LinkLiveSectionModal offerId={id} onClose={() => setShowLinkLiveSectionModal(false)} onLinked={fetchOffer} />}
        {showEditModal && <EditPostModal post={offer} type="offer" onClose={() => setShowEditModal(false)}
          onUpdated={(updatedOffer) => { setOffer(updatedOffer); setShowEditModal(false); }} />}
      </AnimatePresence>
    </div>
  );
}
