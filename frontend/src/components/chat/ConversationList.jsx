        ...
        <motion.button
          ...
          className={`w-full p-4 text-left transition-colors ${
            activeChat?.type === 'dm' && activeChat?.id === conv.id
              ? 'bg-[#fdf3e3] border-l-4 border-[#C97B1A]'
              : 'hover:bg-gray-50 border-l-4 border-transparent'
          }`}
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-[#e18f23] to-[#E8920F] rounded-full flex items-center justify-center text-white font-bold flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                {conv.unread_count > 0 && (
                  <span className="bg-[#fdf3e3]0 text-white text-xs font-bold px-2 py-0.5 rounded-full ml-2">
                    {conv.unread_count}
                  </span>
                )}
              </div>
