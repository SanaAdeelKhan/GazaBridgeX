...
const SKILL_LEVEL_COLORS = { beginner: 'bg-[#fdf3e3] text-green-700', intermediate: 'bg-yellow-100 text-yellow-700', advanced: 'bg-red-100 text-red-700' };
...
export default function LiveSectionCard({ liveSection, index, canDelete, onDelete }) {
  ...
  return (
    ...
    >
      <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-semibold border ${effectiveStatus === 'active' ? 'bg-[#fdf3e3] text-[#1a2e1a] border-emerald-200' : effectiveStatus === 'inactive' ? 'bg-gray-100 text-gray-700 border-gray-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
      ...
      <div className="p-6">
        ...
        <Link to={`/live-sections/${liveSection.id}`}>
          <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-[#C97B1A] transition-colors">{liveSection.title}</h3>
        </Link>
        <div className="mb-4"><p className="text-gray-600 text-sm leading-relaxed">{isExpanded ? liveSection.description : truncated}</p>
          {liveSection.description?.length > 120 && <button onClick={() => setIsExpanded(!isExpanded)} className="text-[#C97B1A] hover:text-[#1a2e1a] text-sm font-medium mt-1">{isExpanded ? 'Show less' : 'Read more'}</button>}
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gradient-to-br from-[#e18f23] to-[#E8920F] rounded-full flex items-center justify-center text-white text-xs font-bold">{liveSection.user_full_name?.split(' ').map(n => n[0]).join('') || 'U'}</div>
          ...
          <div className="flex items-center gap-2">
            <Link to={`/live-sections/${liveSection.id}`} className="px-4 py-2 bg-[#e18f23] hover:bg-[#c97a18] text-white text-sm font-semibold rounded-xl shadow-md hover:shadow-lg transition-all">View</Link>
