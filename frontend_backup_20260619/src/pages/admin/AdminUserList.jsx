    ...
    <div className="max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold" style={{ color: '#3d4a00' }}>
              <span className="text-2xl mr-2">{roleInfo.icon}</span>
              {roleInfo.title}
            </h1>
            <p className="mt-1" style={{ color: '#5a6600' }}>{roleInfo.description}</p>
          </div>
        </div>
        ...
        {/* Table */}
        <div className="rounded-2xl shadow-lg overflow-hidden border" style={{ backgroundColor: '#d8e4f0', borderColor: '#a8c4dc' }}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b" style={{ backgroundColor: '#c4d8ec', borderColor: '#a8c4dc' }}>
                  <th className="text-left px-6 py-4 text-sm font-semibold" style={{ color: '#1e3a5f' }}>Name</th>
                  <th className="text-left px-6 py-4 text-sm font-semibold" style={{ color: '#1e3a5f' }}>Email</th>
                  <th className="text-left px-6 py-4 text-sm font-semibold" style={{ color: '#1e3a5f' }}>Roles</th>
                  {showRoleActions && canManageRoles && (
                    <th className="text-right px-6 py-4 text-sm font-semibold" style={{ color: '#1e3a5f' }}>Actions</th>
                  )}
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  [...Array(5)].map((_, i) => (
                    <tr key={i} className="border-b" style={{ borderColor: '#a8c4dc' }}>
                      <td className="px-6 py-4"><div className="h-4 rounded w-32 animate-pulse" style={{ backgroundColor: '#b8cfe0' }} /></td>
                      <td className="px-6 py-4"><div className="h-4 rounded w-48 animate-pulse" style={{ backgroundColor: '#b8cfe0' }} /></td>
                      <td className="px-6 py-4"><div className="h-4 rounded w-24 animate-pulse" style={{ backgroundColor: '#b8cfe0' }} /></td>
                      {showRoleActions && canManageRoles && (
                        <td className="px-6 py-4"><div className="h-4 rounded w-24 animate-pulse ml-auto" style={{ backgroundColor: '#b8cfe0' }} /></td>
                      )}
                    </tr>
                  ))
                ) : users.length === 0 ? (
                  <tr>
                    <td colSpan={showRoleActions && canManageRoles ? 4 : 3}
                      className="px-6 py-12 text-center" style={{ color: '#5a6600' }}>
                      No users found
                    </td>
                  </tr>
                ) : (
                  users.map(user => (
                    <motion.tr
                      ...
                      style={{ borderColor: '#a8c4dc' }}
                      onMouseEnter={e => e.currentTarget.style.backgroundColor = '#c4d8ec'}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm" style={{ color: '#5a6600' }}>{user.email}</td>
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {user.roles?.map((r, index) => (
                            <span key={index}
                              ...
                              style={
                                r === 'manager' ? { backgroundColor: '#ede9fe', color: '#6d28d9' } :
                                r === 'admin' ? { backgroundColor: '#fef3c7', color: '#92400e' } :
                                r === 'superuser' ? { backgroundColor: '#fee2e2', color: '#991b1b' } :
                                r === 'volunteer' ? { backgroundColor: '#fde8d0', color: '#C26100' } :
                                r === 'seeker' ? { backgroundColor: '#dbeafe', color: '#1e40af' } :
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {['volunteers', 'seekers', 'both'].includes(role) && (
                              <motion.button
                                ...
                                className="px-3 py-1.5 text-white text-xs font-semibold rounded-lg shadow-md transition-all disabled:opacity-50 flex items-center gap-1"
                            ...
                            {role === 'managers' && (
                              <motion.button
                                ...
                                disabled={actionLoading === user.id}
                                className="px-3 py-1.5 text-xs font-semibold rounded-lg transition-all disabled:opacity-50 flex items-center gap-1"
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: '#a8c4dc' }}>
              ...
              <span className="text-sm" style={{ color: '#5a6600' }}>Page {page} of {totalPages}</span>
        ...
        {/* Legend */}
        {showRoleActions && canManageRoles && (
          <div className="mt-6 p-4 rounded-2xl shadow-lg border" style={{ backgroundColor: '#d8e4f0', borderColor: '#a8c4dc' }}>
            <h3 className="text-sm font-semibold mb-3" style={{ color: '#1e3a5f' }}>Role Management</h3>
            <div className="flex flex-wrap gap-4 text-sm" style={{ color: '#5a6600' }}>
              {['volunteers', 'seekers', 'both'].includes(role) && (
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1.5 text-white text-xs font-semibold rounded-lg"
                    style={{ background: 'linear-gradient(to right, #7c3aed, #ec4899)' }}>Promote</span>
              ...
              {role === 'managers' && (
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1.5 text-xs font-semibold rounded-lg"
                    style={{ backgroundColor: '#fee2e2', color: '#991b1b' }}>Demote</span>
                  <span>Removes Manager role from user</span>
                </div>
              )}
            </div>
            <p className="text-xs mt-3" style={{ color: '#5a6600' }}>
