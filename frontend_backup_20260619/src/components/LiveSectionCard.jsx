...
export default function LiveSectionCard({ liveSection, index, canDelete, onDelete }) {
  ...
  return (
    ...
    >
      <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-semibold border ${effectiveStatus === 'active' ? 'bg-orange-500 text-orange-600 border-orange-400' : effectiveStatus === 'inactive' ? 'bg-gray-100 text-gray-700 border-gray-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
      ...
      <div className="p-6">
        ...
        <Link to={`/live-sections/${liveSection.id}`}>
          <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-orange-600 transition-colors">{liveSection.title}</h3>
        </Link>
        <div className="mb-4"><p className="text-gray-600 text-sm leading-relaxed">{isExpanded ? liveSection.description : truncated}</p>
          {liveSection.description?.length > 120 && <button onClick={() => setIsExpanded(!isExpanded)} className="text-orange-600 hover:text-orange-600 text-sm font-medium mt-1">{isExpanded ? 'Show less' : 'Read more'}</button>}
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gradient-to-br from-orange-600 to-orange-500 rounded-full flex items-center justify-center text-white text-xs font-bold">{liveSection.user_full_name?.split(' ').map(n => n[0]).join('') || 'U'}</div>
          ...
          <div className="flex items-center gap-2">
            <Link to={`/live-sections/${liveSection.id}`} className="px-4 py-2 bg-gradient-to-r from-orange-600 to-orange-500 text-white text-sm font-semibold rounded-xl shadow-md hover:shadow-lg transition-all">View</Link>
